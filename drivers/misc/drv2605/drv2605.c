...
static int drv2605_reg_write(struct drv2605_data *pDrv2605data, unsigned char reg, char val)
{
	...
	s32 ret;
	ret = i2c_smbus_write_byte_data(pDrv2605data->client, reg, val);
	drv26xx_debug("%s write %x = %x------------> %d\n", __FUNCTION__, reg, val, ret);
	return ret;
}
...
{
	int reval = -ENOMEM;
	drv26xx_debug(KERN_ERR"into Haptics_init\n"); 
...
static int drv2605_probe(struct i2c_client* client, const struct i2c_device_id* id)
{
	...
	drv2605_plat_data. GpioEnable= of_get_named_gpio(of_node, "enable-gpio", 0);
