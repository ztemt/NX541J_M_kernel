...
static int init_teei_framework(void)
{
	...
	if (soter_error_flag == 1) {
		printk("[%s][%d]soter_error_flag == 1",__func__,__LINE__);
		return -1;
	}
	...
	if (retVal == -1) {
		printk("[%s][%d]teei_service_init_first failed,retVal == -1 ",__func__,__LINE__);
		return -1;
	}
	...
	printk("[%s][%d]boot_decryto_lock before ",__func__,__LINE__);
	down(&boot_decryto_lock);
	up(&boot_decryto_lock);
	printk("[%s][%d]boot_decryto_lock after ",__func__,__LINE__);
	retVal = teei_service_init_second();
	if (retVal == -1) {
		printk("[%s][%d]teei_service_init_second failed,retVal == -1 ",__func__,__LINE__);
		return -1;
	}
