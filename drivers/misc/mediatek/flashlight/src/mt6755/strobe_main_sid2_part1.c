...
*/
	case FLASH_IOC_SET_REG_ADR:
		pr_err("FLASH_IOC_SET_REG_ADR: %d\n", (int)arg);
		/* g_reg = arg; */
		break;
	case FLASH_IOC_SET_REG_VAL:
		pr_err("FLASH_IOC_SET_REG_VAL: %d\n", (int)arg);
		/* g_val = arg; */
		break;
	...
	case FLASH_IOC_GET_REG:
		pr_err("FLASH_IOC_GET_REG: %d\n", (int)arg);

		/* i4RetValue = valTemp; */
		//pr_err("FLASH_IOC_GET_REG: v=%d\n", valTemp);
		break;
...
extern int flashEnable_KD2684_2(void);
extern int flashDisable_KD2684_2(void);
extern int setDuty_KD2684_2(int duty);
...
//	kal_uint8 valTemp;
	...
	iow_shift = cmd - (_IOW(FLASHLIGHT_MAGIC, 0, int));
	iowr_shift = cmd - (_IOWR(FLASHLIGHT_MAGIC, 0, int));
	pr_err("constant_flashlight_ioctl() line=%d ior_shift=%d, iow_shift=%d iowr_shift=%d arg=%d\n",
	     __LINE__, ior_shift, iow_shift, iowr_shift, (int)arg);
	switch (cmd) {
	...
	case FLASH_IOC_SET_ONOFF:
		pr_err("FLASHLIGHT_ONOFF 2222: %d\n", (int)arg);
		if (arg == 1) {
			if (g_timeOutTimeMs != 0) {
				ktime_t ktime;
