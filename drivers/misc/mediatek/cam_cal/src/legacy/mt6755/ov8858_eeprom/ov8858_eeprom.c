...
static int iWriteData(unsigned int  ui4_offset, unsigned int  ui4_length,unsigned char pinputdata)
{
	...
	//CAM_CALDB("[CAM_CAL2] not implemented!");
	 kdSetI2CSpeed(EEPROM_I2C_SPEED); // Add this func to set i2c speed by each sensor
...
static void imx179_stream_on(void)
{
...
#endif
{
	...
	case CAM_CALIOC_S_WRITE:
		...
		i4RetValue = iWriteData((unsigned int)ptempbuf->u4Offset, (unsigned int)ptempbuf->u4Length, (unsigned char)*pu1Params);
		imx179_stream_on();
              mdelay(2);
#ifdef CAM_CALGETDLT_DEBUG
