...
#ifdef DEBUG_THREAD
#endif
#ifdef ztemt_audio_headset_eu_us_support
static unsigned int pin_eu_us_switch = (116| 0x80000000);
static unsigned int state_eu_us_switch=0;//0 stands for us, 1 stands for eu
#endif
...
#define PIN_ADC_CHANNEL 5
	...
	case PLUG_OUT:
		...
		if (current_status == 0) {
			...
			if (1 == eint_accdet_sync_flag) {
#ifdef ztemt_audio_headset_eu_us_support
				if(state_eu_us_switch==0){
					pr_err("nubia [Accdet] set pin_eu_us_switch %d ON \n",(pin_eu_us_switch&0xffff));
					mt_set_gpio_mode(pin_eu_us_switch, GPIO_MODE_00);	/* GPIO117: DPI_D3, mode 0 */
					mt_set_gpio_pull_enable(pin_eu_us_switch, GPIO_PULL_ENABLE);
					mt_set_gpio_dir(pin_eu_us_switch, GPIO_DIR_OUT);	/* output */
					mt_set_gpio_out(pin_eu_us_switch, GPIO_OUT_ONE);	/* low disable */
					msleep(500);
					state_eu_us_switch=1;

					pr_err("nubia [Accdet] switch to eu %d mode \n",state_eu_us_switch);
				}
				else{
					pr_err("nubia [Accdet] set pin_eu_us_switch %d off \n",(pin_eu_us_switch&0xffff));
					mt_set_gpio_mode(pin_eu_us_switch, GPIO_MODE_00);	/* GPIO117: DPI_D3, mode 0 */
					mt_set_gpio_pull_enable(pin_eu_us_switch, GPIO_PULL_ENABLE);
					mt_set_gpio_dir(pin_eu_us_switch, GPIO_DIR_OUT);	/* output */
					mt_set_gpio_out(pin_eu_us_switch, GPIO_OUT_ZERO);	/* low disable */
					msleep(500);
					state_eu_us_switch=0;
					r_err("nubia [Accdet] switch to us %d mode \n",state_eu_us_switch);
				}
#endif
				cable_type = HEADSET_NO_MIC;
				accdet_status = HOOK_SWITCH;
			...
			if (1 == eint_accdet_sync_flag) {
				...
				/*AB=11 debounce=30ms*/
#ifdef ztemt_audio_headset_eu_us_support
				pr_err("nubia [Accdet] headsest type is %d (0 stands for us, 1 stands for eu) \n",state_eu_us_switch);
#endif
