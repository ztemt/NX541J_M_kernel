...
#include <linux/platform_device.h>   //by qliu4244 add sensors function 
#include "lsm6ds3-int.h"


#define POWER_NONE_MACRO MT65XX_POWER_NONE
#define CHOSE_MTK_WEHT_CALI
...
struct mutex st_dma_mutex;
DEFINE_MUTEX(st_dma_mutex);
...
static int LSM6DS3_acc_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	else
	{
		...
		databuf[0] |= LSM6DS3_ACC_ODR_26HZ;
	}
	databuf[1] = databuf[0];
...
/* do reset to ensure hardware inital state -- add by liaoxl.st 8.27.2005 end */
    ...
    res = LSM6DS3_enable_pedo(client, true);
    ...
    res =hwmsen_write_byte(client,0x0f,0x8e);
    if(res<0){
        return res;

    }
    res =hwmsen_write_byte(client,0x14,0x5f);
    if(res<0){
        return res;
...
static int lsm6ds3_step_c_get_data(u32 *value, int *status)
{
#if 0
	int err = 0;
	u16 pedo_data = 0;
...
static int __init lsm6ds3_init(void)
{
   ...
   /*     i2c_lsm6ds3.addr =hw->i2c_addr[0];
