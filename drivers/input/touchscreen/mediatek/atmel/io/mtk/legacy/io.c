...
void device_regulator_enable(struct device *dev)
{
	...
	tpd_gpio_output(pdata->gpio_reset, 0);
	...
	tpd_gpio_output(pdata->gpio_reset, 1);
...
int device_power_init(struct device *dev)
{
	...
	pdata->reg_vdd = regulator_get(dev, "vldo28");
#endif
#if defined (CONFIG_MTK_TP_AVDD_LDO_VCN33)
        pdata->reg_vdd = regulator_get(dev, "vcn33_bt");
#endif
