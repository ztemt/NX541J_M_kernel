			...
			irq_handler_t thread_fn, const char *devname, void *dev_id)
{
	...
	return
		...
		request_irq(pdata->irq, handler, pdata->irqflags, devname, dev_id);
#else
		request_threaded_irq(pdata->irq, NULL, thread_fn, pdata->irqflags, devname, dev_id);
#endif
}
...
int device_parse_default_dts(struct device *dev)
{
	struct mxt_platform_data *pdata = dev_get_platdata(dev);
	int error = 0;
...
void device_release_dts(struct device *dev)
{
	struct mxt_platform_data *pdata = dev_get_platdata(dev);
