...
int mxt_irq;
unsigned int mxtgpiopin,mxtdebounce;
...
irqreturn_t  mxt_irq_handler(int irq,void * data)
{
	//TPD_DEBUG("TPD interrupt has been triggered\n");
	//printk("mxt_irq_handler sucess \n");
	tpd_flag = 1;
	g_data = data;
	g_irq = irq;
...
static void mxt_proc_t15_messages(struct mxt_data *data, u8 *msg)
{
	...
	dev_warn(&data->client->dev, "check empty\n");

	if (keylist_empty(pdata, MXT_TOUCH_KEYARRAY_T15))
		return;

	dev_info(&data->client->dev, "check key status %lx %lx\n", keystates, data->t15_keystatus);
	...
	if (key_chg) {
		for (i = 0;; i++) {
			if(key_chg & 0x1) {
				...
				dev_info(&data->client->dev, "key[%d] value %d\n", i, value);
				if (value > 0) {
...
static ssize_t mxt_touch_mode_show(struct device *dev,
                   struct device_attribute *attr, char *buf)
{
    return scnprintf(buf,PAGE_SIZE,"touch mode %d\n",touch_mode);
}
...
static DEVICE_ATTR(update_fw0, S_IWUSR, NULL, mxt_update_fw_store);
...
static DEVICE_ATTR(bootloader, S_IWUSR | S_IRUSR, mxt_bootloader_show, mxt_bootloader_store);
static DEVICE_ATTR(t19, S_IWUSR | S_IRUSR, mxt_t19_gpio_show, mxt_t19_gpio_store);
static DEVICE_ATTR(t25, S_IWUSR | S_IRUSR, mxt_t25_selftest_show, mxt_t25_selftest_store);
static DEVICE_ATTR(cmd, S_IWUSR, NULL,mxt_cmd_store);
static DEVICE_ATTR(depth, S_IWUSR | S_IRUSR, mxt_irq_depth_show, mxt_irq_depth_store);
...
static DEVICE_ATTR(easy_wakeup_gesture, S_IWUSR|S_IXUSR, NULL, mxt_easy_wakeup_gesture_store);
...
static DEVICE_ATTR(plugin_tag, S_IRUGO, mxt_plugin_tag_show, NULL);
#	if defined(CONFIG_MXT_PLIGIN_CAL)
	static DEVICE_ATTR(cal, S_IWUSR | S_IRUSR, mxt_plugin_cal_show, mxt_plugin_cal_store);
#	endif
static DEVICE_ATTR(en_glove, S_IWUSR | S_IRUSR, mxt_plugin_glove_show, mxt_plugin_glove_store);
static DEVICE_ATTR(en_stylus, S_IWUSR | S_IRUSR, mxt_plugin_stylus_show, mxt_plugin_stylus_store);
static DEVICE_ATTR(en_gesture, S_IWUSR | S_IRUSR, mxt_plugin_wakeup_gesture_show, mxt_plugin_wakeup_gesture_store);
static DEVICE_ATTR(gesture_list, S_IWUSR | S_IRUSR, mxt_plugin_gesture_list_show, mxt_plugin_gesture_list_store);
static DEVICE_ATTR(gesture_trace, S_IRUSR, mxt_plugin_gesture_trace_show, NULL);
#	if defined(CONFIG_MXT_PLIGIN_MISC)
	static DEVICE_ATTR(misc, S_IWUSR | S_IRUSR, mxt_plugin_misc_show, mxt_plugin_misc_store);
#	endif
#	if defined(CONFIG_MXT_PLIGIN_CLIP)
	static DEVICE_ATTR(clip, S_IWUSR | S_IRUSR, mxt_plugin_clip_show, mxt_plugin_clip_store);
					...
					const struct i2c_device_id *id)
{
	...
	dev_err(dev, "%s: driver version 0x%x\n", 
	...
	snprintf(data->phys, sizeof(data->phys), "i2c-%u-%04x/input0",
       ...
       dev_err(dev, "%s: driver version1 0x%x\n", 
			__func__,DRIVER_VERSION);
	data->client = client;
...
#endif
      // printk("add by qiang mxt_handle_pdata");
