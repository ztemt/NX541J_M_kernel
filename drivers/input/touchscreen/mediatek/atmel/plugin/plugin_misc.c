...
#define MSC_FLAG_RESETING				P_FLAG_EV_RESETING
#define MSC_FLAG_CALING					P_FLAG_EV_CALING
#define MSC_FLAG_WORKAROUND_HALT		P_FLAG_EV_HALT

#define MSC_FLAG_RESET					P_FLAG_EV_RESET
#define MSC_FLAG_CAL					P_FLAG_EV_CAL
#define MSC_FLAG_RESUME					P_FLAG_EV_RESUME
...
#define MSC_FLAG_MASK_LOW			P_FLAG_EV_DONE_MASK
...
static void plugin_misc_hook_t6(struct plugin_misc *p, u8 status)
{
	...
	plugin_p_hook_t6(status, &obs->flag,MSC_FLAG_MASK_NORMAL);

	//dev_dbg(dev, "mxt msc flag=0x%lx %x\n",
	//	 obs->flag, status);
}
...
void plugin_interface_misc_deinit(struct plugin_misc *p)
{
