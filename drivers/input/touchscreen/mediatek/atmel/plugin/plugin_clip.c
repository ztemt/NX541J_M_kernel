...
#define CLP_FLAG_RESETING				P_FLAG_EV_RESETING
#define CLP_FLAG_CALING					P_FLAG_EV_CALING
#define CLP_FLAG_WORKAROUND_HALT		P_FLAG_EV_HALT

#define CLP_FLAG_RESET					P_FLAG_EV_RESET
#define CLP_FLAG_CAL						P_FLAG_EV_CAL
#define CLP_FLAG_RESUME					P_FLAG_EV_RESUME
...
#define CLP_FLAG_MASK_LOW			P_FLAG_EV_DONE_MASK
...
static void plugin_clip_hook_t6(struct plugin_clip *p, u8 status)
{
	...
	plugin_p_hook_t6(status, &obs->flag,CLP_FLAG_MASK_NORMAL);

	//dev_dbg(dev, "mxt clp flag=0x%lx %x\n",
	//	 obs->flag, status);
}
...
void plugin_interface_clip_deinit(struct plugin_clip *p)
{
