...
#define CR_FLAG_RESETING				P_FLAG_EV_RESETING
#define CR_FLAG_CALING					P_FLAG_EV_CALING
#define CR_FLAG_WORKAROUND_HALT			P_FLAG_EV_HALT

#define CR_FLAG_RESET					P_FLAG_EV_RESET
#define CR_FLAG_CAL						P_FLAG_EV_CAL
#define CR_FLAG_RESUME					P_FLAG_EV_RESUME
...
#define CR_FLAG_MASK_LOW			P_FLAG_EV_DONE_MASK
...
static void plugin_cal_hook_t6(struct plugin_cal *p, u8 status)
{
	...
	plugin_p_hook_t6(status, &obs->flag,CR_FLAG_MASK_NORMAL);
...
static int init_cal_obj_list(struct plugin_cal *p, struct list_head *head, int obj_size)
{
	...
	//dev_dbg(dev, "allocated CAL buffer at %p, node number %d\n",
		//	buf,size);
	obs->obs_buf = buf;
...
void plugin_interface_cal_deinit(struct plugin_cal *p)
{
