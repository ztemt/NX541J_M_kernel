...
#define PI_FLAG_RESETING				P_FLAG_EV_RESETING
#define PI_FLAG_CALING					P_FLAG_EV_CALING
#define PI_FLAG_WORKAROUND_HALT			P_FLAG_EV_HALT

#define PI_FLAG_RESET					P_FLAG_EV_RESET
#define PI_FLAG_CAL						P_FLAG_EV_CAL
#define PI_FLAG_RESUME					P_FLAG_EV_RESUME
...
static int plugin_proci_pi_hook_t61(void *pi_id, u8 *msg, unsigned long pl_flag)
{
	...
	//below is test code for timer wakeup

#if defined(CONFIG_T61_TIMER_WAKEUP)
	if (msg[0] == T61_TIMER_ID_PI) {	//dedicate a timer ID
...
static int plugin_proci_pi_wakeup_enable(struct plugin_proci *p, unsigned long pl_flag)
{
	...
	if (test_and_clear_flag(PI_FLAG_WAKEUP, &obs->flag))//some error occur in suspend, reset chip
		dev_err(dev, "set wakeup enable loop, warning\n");
...
static struct reg_config mxt_dwakeup_cfg[] = {
	...
	{.reg = MXT_GEN_ACQUISITIONCONFIG_T8, .instance = 0, 
		.offset = 14,.buf = {1}, .len = 1, .mask = 0x0,.flag = BIT_MASK(OP_SET)},
	...
	{.reg = MXT_SPT_CTECONFIG_T46,
		.offset = 2,.buf = {8}, .len = 1, .mask = 0,.flag = BIT_MASK(OP_SET)},
	...
	{ .reg = MXT_PROCI_RETRANSMISSIONCOMPENSATION_T80,
		.offset = 9,.buf = {15}, .len = 1, .mask = 0,.flag = BIT_MASK(OP_SET)},
	...
	{.reg = MXT_TOUCH_PTC_KEYS_T97, .instance = 0, 
		.offset = 0,.buf = {0}, .len = 0, .mask = 0x2,.flag = BIT_MASK(OP_SET)},
	...
	{.reg = MXT_PROCG_NOISESUPSELFCAP_T108, .instance = 0,
                .offset = 0,.buf = {0}, .len = 1, .mask = 0x1,.flag = BIT_MASK(OP_SET)},
	...
	{.reg = MXT_TOUCH_MULTITOUCHSCREEN_T100,
		.offset = 0,.buf = {201}, .len = 1, .mask = 0,.flag = BIT_MASK(OP_SET)},

	{.reg = MXT_GEN_ACQUISITIONCONFIG_T8, .instance = 0, 
		.offset = 10,.buf = {10,2,2}, .len = 3, .mask = 0x0,.flag = BIT_MASK(OP_SET)},

       {.reg = MXT_GEN_ACQUISITIONCONFIG_T8, .instance = 0, 
               .offset = 2,.buf = {20,20}, .len = 2, .mask = 0x0,.flag = BIT_MASK(OP_SET)},

       {.reg = MXT_PROCI_GRIPSUPPRESSION_T40, .instance = 0, 
               .offset = 0,.buf = {1,100,100,100,100}, .len = 5, .mask = 0,.flag = BIT_MASK(OP_SET)},
	{.reg = MXT_GEN_POWERCONFIG_T7,
		.offset = 0,.buf = {70,10,5}, .len = 3, .mask = 0,.flag = BIT_MASK(OP_SET)},

	{.reg = MXT_PROCI_TOUCHSUPPRESSION_T42,
		.offset = 0,.buf = {0}, .len = 0, .mask = 0,.flag = BIT_MASK(OP_SET)},
		...
		if (count > 6 && count < sizeof(name)) {
			...
			if (ret == 1) {
				if (strncmp(name, "config", 6) == 0) {
					...
					if (ret == 2) {
						...
						if (i >= 0 && i < PI_LIST_NUM) {
							if (j >= 0 && j < cfg->num_reg_cfg[i]) {
								...
								ofs = 0;
								ret = sscanf(buf + offset, "T%hhd(%hhd) offset %hd len %hd(%lx,%lx,%hhd): %n", 
					...
					ofs = 0;
					ret = sscanf(buf + offset, "T%hhd offset %hd len %hd: %n", 
...
void plugin_interface_proci_deinit(struct plugin_proci *p)
{
