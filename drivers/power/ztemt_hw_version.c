...
#ifdef CONFIG_ZTEMT_HW_CHIP_SERIAL
#include <linux/io.h>
#endif
extern int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata);

#ifdef CONFIG_ZTEMT_HW_CHIP_SERIAL
static uint32_t    g_chip_serial_num;
#endif

#ifndef CONFIG_ZTEMT_HW_VERSION_DEBUG
static int debug_value=1;
#else
static int debug_value=0;
#endif
#define ztemt_hw_version_debug(fmt, args...) do {if(debug_value==1)printk(KERN_ERR "[ztemt_hw_version]"fmt, ##args);} while(0)
...
/*************************************************************************
*************************add ztemt mtk*************************************/
#ifndef FEATURE_ZTEMT_HW_CONFIG_BY_GPIO
int ztemt_get_adc_value(int channel)
{
   // int hw_id_uv = 0;

	//如果默认电压没有打开，按channel来达到对应的ldo
  //  hw_id_uv = pm8x41_adc_channel_read(channel);
  	int data[4] = {0,0,0,0};
	int res =0;
	int rawdata=0;
	int hw_vol = 0;

	res =IMM_GetOneChannelValue(channel,data,&rawdata);
	if(res<0) {
		ztemt_hw_version_debug("ztemt_hw_pcb_gpio_ADC error !!!\n");
		return 0; 
	} else {
		hw_vol = data[0]*1000+data[1]*10;
		ztemt_hw_version_debug("ztemt_hw_pcb_gpio_ADC hw_vol = %d\n",hw_vol);
	}
	ztemt_hw_version_debug("ztemt_hw_pcb_gpio_ADC channel = %d\n",channel);
	ztemt_hw_version_debug("ztemt_hw_pcb_gpio_ADC = %d\n",hw_vol);
       return hw_vol;
}
#endif
/*************************************************************************
...
#ifdef CONFIG_ZTEMT_HW_CHIP_SERIAL
static void ztemt_serial_num_init( void )
{
/*Check the spec to get address on the plateform you use*/
#define QFPROM_SERIAL_NUM_LSB  0xA60A8
#define QFPROM_SERIAL_NUM_MSB

        void __iomem *serial;

        serial = ioremap(QFPROM_SERIAL_NUM_LSB, 4);
        if( !serial ) {
            pr_err("could not map QFPROM_SERIAL_NUM_LSB address\n");
            g_chip_serial_num = 0;
        }else {
            g_chip_serial_num=readl_relaxed(serial);
            ztemt_hw_version_debug("init chip_serial_num=%x\n", g_chip_serial_num);
            iounmap(serial);
        }
        return ;
 }

static uint32_t ztemt_get_serial_num( void )
{
    if ( g_chip_serial_num == 0 )
        ztemt_serial_num_init();

    ztemt_hw_version_debug("read cached chip_serial_num=%x\n", g_chip_serial_num);
    return g_chip_serial_num;
}
#endif
...
#ifdef CONFIG_ZTEMT_HW_CHIP_SERIAL
static ssize_t ztemt_chip_serial_show(struct kobject *kobj,
	   struct kobj_attribute *attr, char *buf)
{
    uint32_t chip_num = ztemt_get_serial_num();
    return sprintf(buf,"0x%x",chip_num);
}

static struct kobj_attribute chip_serial_attr=
    __ATTR(chip_serial, 0664, ztemt_chip_serial_show, NULL);
#endif
