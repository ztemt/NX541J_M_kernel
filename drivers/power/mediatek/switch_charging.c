...
#if defined(CONFIG_MTK_DYNAMIC_BAT_CV_SUPPORT) && defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
	...
	} else if (g_temp_status == TEMP_POS_45_TO_POS_60 && BMT_status.temperature >= TEMP_POS_45_THRESHOLD) {
...
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
	} else if (BMT_status.temperature >= (TEMP_POS_45_THRESHOLD - BMT_status.temperature_delta)) {	/* control 45c to normal behavior */
#else
...
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
					TEMP_POS_45_THRESHOLD - BMT_status.temperature_delta, TEMP_POS_60_THRESHOLD);
#else
...
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
		BMT_status.batt_health = POWER_SUPPLY_HEALTH_WARM;
#endif
		}
	} else if (BMT_status.temperature >= TEMP_POS_10_THRESHOLD) {
		if (((g_temp_status == TEMP_POS_45_TO_POS_60)
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
			 && (BMT_status.temperature >= (TEMP_POS_45_THRES_MINUS_X_DEGREE - BMT_status.temperature_delta)))
#else
...
#ifdef CONFIG_NUBIA_PM_COMMON_FEATURE
					TEMP_POS_10_THRESHOLD, TEMP_POS_45_THRESHOLD - BMT_status.temperature_delta);
#else
...
static void set_jeita_charging_current(void)
{
	...
	if ((g_temp_status == TEMP_NEG_10_TO_POS_0) || (g_temp_status == TEMP_POS_0_TO_POS_10)){
