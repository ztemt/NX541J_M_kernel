...
int mt63xx_codec_prepare_forak4376(struct snd_pcm_substream *substream, struct snd_soc_dai *Daiport)
{
        if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
                pr_warn("lee:mt63xx_codec_prepare set up SNDRV_PCM_STREAM_CAPTURE rate = %d\n",
                        substream->runtime->rate);
                mBlockSampleRate[AUDIO_ANALOG_DEVICE_IN_ADC] = substream->runtime->rate;

        } else if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
                pr_warn("lee:mt63xx_codec_prepare set up SNDRV_PCM_STREAM_PLAYBACK rate = %d\n",
                        substream->runtime->rate);
                mBlockSampleRate[AUDIO_ANALOG_DEVICE_OUT_DAC] = substream->runtime->rate;
                if(16000 == substream->runtime->rate)
                    mBlockSampleRate[AUDIO_ANALOG_DEVICE_IN_ADC] = substream->runtime->rate;
        }
        return 0;
...
#if defined(CONFIG_MTK_LEGACY)
#ifdef CONFIG_ZTEMT_AUDIO_AW8738
