...
static int ak4376_pdn_control(struct snd_soc_codec *codec, int pdn)
{
	...
	if ((ak4376->pdn1 == 0) && (pdn == 1)) {
		mt_set_gpio_out(ak4376->priv_pdn_en , GPIO_OUT_ONE);
		akdbgprt("\t[AK4376] %s(%d) Turn on priv_pdn_en %d\n", __FUNCTION__,__LINE__,ak4376->priv_pdn_en);
		ak4376->pdn1 = 1;
		ak4376->pdn2 = 1;
		udelay(800);
		...
		mt_set_gpio_out(ak4376->priv_pdn_en , GPIO_OUT_ZERO);
		akdbgprt("\t[AK4376] %s(%d) Turn off priv_pdn_en%d \n", __FUNCTION__,__LINE__,ak4376->priv_pdn_en);
		ak4376->pdn1 = 0;
		ak4376->pdn2 = 0;
...
unsigned int ak4376_read_byte(u8 cmd, u8 *returnData)
{
...
static int ak4376_codec_prepare(struct snd_pcm_substream *substream, struct snd_soc_dai *dai)
{
        mt63xx_codec_prepare_forak4376(substream,dai);
        return 0;
}
...
static int ak4376_probe(struct snd_soc_codec *codec)
{
	...
	ak4376->fs1 = 48000;
	ak4376->rclk = 0;
	ak4376->nBickFreq = 2;		//0:32fs, 1:48fs, 2:64fs
                            ...
                            const struct i2c_device_id *id)
{
	...
	ak4376->pdn2 = 0;
        ...
        ret = of_property_read_u32(i2c->dev.of_node, "ldo-en",&ak4376->priv_ldo_en);
        if(ret < 0){
            pr_err("%s: Can not get ldo-en gpio\n", __FUNCTION__);
        }

        if(ak4376->priv_ldo_en <= 0)
            pr_err("%s: Error to get ldo-en gpio\n", __FUNCTION__);
        else{
            ak4376->priv_ldo_en |= 0x80000000;
            mt_set_gpio_mode(ak4376->priv_ldo_en , GPIO_MODE_00);
            mt_set_gpio_dir(ak4376->priv_ldo_en , GPIO_DIR_OUT);
            mt_set_gpio_out(ak4376->priv_ldo_en , GPIO_OUT_ONE);
        }

        ret = of_property_read_u32(i2c->dev.of_node, "pdn-en",&ak4376->priv_pdn_en);
        if(ret < 0){
            pr_err("%s: Can not get pdn-en gpio\n", __FUNCTION__);
        }

        if(ak4376->priv_pdn_en <= 0){
            pr_err("%s: Error to get pdn-en gpio\n", __FUNCTION__);
        }
        else{
            ak4376->priv_pdn_en |= 0x80000000;
            mt_set_gpio_mode(ak4376->priv_pdn_en , GPIO_MODE_00);
	    mt_set_gpio_dir(ak4376->priv_pdn_en , GPIO_DIR_OUT);
	    mt_set_gpio_out(ak4376->priv_pdn_en , GPIO_OUT_ONE);
        }
	...
	akdbgprt("\t[AK4376] %s(%d) register codec ok\n", __FUNCTION__,__LINE__);
...
static int ak4376_platform_probe(struct platform_device *pdev)
{
	...
	if(ret !=0)
	{
		printk("[ak4376] %s(%d) probe failed\n", __FUNCTION__, __LINE__);
		return -1;
	}
	return 0;
}
...
static int __init ak4376_modinit(void)
{
	...
	printk("[ak4376] %s(%d) pre register platform device\n", __FUNCTION__, __LINE__);
	ret = platform_device_register(&ak4376_pdevice);
	if(ret){
		printk("[ak4376] %s(%d) platform_device_register failed!\n", __FUNCTION__, __LINE__);
		return ret;
	}
...
//		printk("[tfa98xx] %s(%d) name:%s, addr:%i\n", __FUNCTION__, __LINE__, devinfo->board_info.type, devinfo->board_info.addr);
//	}

	printk("[ak4376] %s(%d) pre register platform driver\n", __FUNCTION__, __LINE__);
	if(platform_driver_register(&ak4376_platform_driver)){
		printk("[ak4376] %s(%d) platform_driver_register failed\n", __FUNCTION__, __LINE__);
		return -ENODEV;
	}
    return ret;
